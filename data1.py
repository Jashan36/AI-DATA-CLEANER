...
# --- Web Scraping and NLP Utilities ---
...
def fetch_url_content(url, timeout=30, retries=3, delay=3):
    headers = {
    ...
    for attempt in range(retries):
        try:
            ...
            # Show status and partial content for debugging
            if resp.status_code != 200:
                return f"ERROR: HTTP {resp.status_code} - {resp.reason}\nFirst 500 chars: {resp.text[:500]}"
            return resp.text
        except Exception as e:
            last_exception = e
            time.sleep(delay)
    return f"ERROR: {last_exception} (after {retries} retries, timeout={timeout}s)"

def parse_and_extract_info(html, url):
    # Try lxml, fallback to html.parser if not available
    try:
    ...
    # Clean main text: remove very short lines and repeated headers
    ...
    try:
        ...
        pass
    # Extract About/Contact sections (get full paragraph or section)
    ...
    if nlp is not None and main_text_clean:
        doc = nlp(main_text_clean)
        # Get first 2-3 sentences for summary
        sents = list(doc.sents)
        summary = " ".join([sent.text for sent in sents[:3]])
        # Filter entities for ORG, GPE, PERSON, EMAIL, PHONE
        wanted_labels = {"ORG", "GPE", "PERSON", "EMAIL", "PHONE", "LOC"}
